...
class SpeckleSpacklePlay extends Component {
	...
	updateData(response) {
		...
		receivedGameInfo.gameGrid = this.resetSquares([
			...receivedGameInfo.gameGrid
		]);
		this.props.updateCurrentPath(
			"speckle_spackle_play",
			receivedData.puzzle_name,
			"play",
			[this.evaluateAnswer]
		);
		this.setState({
	...
	evaluateAnswer() {
		...
		if (
			...
			let missing = false;
			let clue = false;
			const arrayOfArrays = rowLog.concat(
				columnLog,
				leftLog,
				rightLog,
				topLog,
			...
			for (let i = 0; i < arrayOfArrays.length; i++) {
				switch (arrayOfArrays[i].errorType) {
					...
					case "missing":
						...
						const missingColors = document.getElementsByClassName(
							arrayOfArrays[i].location
						);
	...
	checkLeftClues(gameInfo, outerGridSize) {
		let leftLog = [];
		for (
			let i = outerGridSize, row = 1;
			i < gameInfo.gameGrid.length - outerGridSize;
			i += outerGridSize, row++
		) {
			if (gameInfo.gameGrid[i]["opacity"] === 1) {
				...
				let k = i + 1;
				while (
					gameInfo.gameGrid[k]["colorNum"] === "color0" &&
					k < i + gameInfo.gridSize
				) {
					k++;
				}
				if (
					gameInfo.gameGrid[i]["colorNum"] !==
					gameInfo.gameGrid[k]["colorNum"]
				) {
	...
	checkRightClues(gameInfo, outerGridSize) {
		let rightLog = [];
		for (
			let i = outerGridSize * 2 - 1, row = 1;
			i < gameInfo.gameGrid.length - 2;
			i += outerGridSize, row++
		) {
			if (gameInfo.gameGrid[i]["opacity"] === 1) {
				let clueColor = gameInfo.gameGrid[i]["colorNum"];
	...
	checkTopClues(gameInfo, outerGridSize) {
		...
		for (let i = 1; i <= gameInfo.gridSize; i++) {
			if (gameInfo.gameGrid[i]["opacity"] === 1) {
				...
				while (
					gameInfo.gameGrid[k]["colorNum"] === "color0" &&
					k < gameInfo.gameGrid.length - outerGridSize
				) {
					k += outerGridSize;
				}
				if (
	...
	checkBottomClues(gameInfo, outerGridSize) {
		...
		for (
			let i = gridLength - gameInfo.gridSize - 1, column = 1;
			i < gridLength - 1;
			i++, column++
		) {
			if (gameInfo.gameGrid[i]["opacity"] === 1) {
	...
	isEachColorInEveryRowOnce() {
		...
		for (let i = 1; i <= gridSize; i++) {
			...
			for (
				let k = i * (gridSize + 2) + 1;
				k <= i * (gridSize + 2) + gridSize;
				k++
			) {
				if (gameGrid[k].colorNum !== "color0") {
					const indexOfColor = colorArray.indexOf(
						gameGrid[k].colorNum
					);
					if (indexOfColor === -1) {
						rowLog.push({
