...
class Login extends Component {
	...
	facebookLogin(event) {
		...
		FB.login(
			function(response) {
				console.log(
					"We got a response from FB.login and it is: ",
					response
				);
				if (response.status === "connected") {
					...
					Axios.post("/login", {
						...
						const token =
							receivedData.response.authResponse.accessToken;
	...
	render() {
		...
		return (
			<div>
				<PageTitle
					backgroundImg="forestvalley"
					color="white"
					text="LOGIN"
					subText="enter your credentials below"
				/>
				<div className="container mt-5 text-center">
					<form
						onSubmit={e => this.facebookLogin(e)}
						className={`form-group ${error ? "has-danger" : ""}`}
					>
						...
						/>
						<small
							className={`form-control-feedback text-left ${error
								? ""
								: "d-none"}`}
						>
