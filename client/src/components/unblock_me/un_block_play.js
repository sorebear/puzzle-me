...
//GETS CALLED FROM STATE TO CREATE PIECE MAP FROM PROPS OBJ
       function createPieceMap(props){
           var starterPiece = {y: 2, x: 0, type: "unBlock_starterPiece", width: 2, height: 1};
    ...
    handleDraggingY(ev){
        ...
        if(amountToMove > 0 && amountToMove < this.state.oneBoardUnit*2 && this.state.canMoveDown === true){
            ...
            this.state.pieceMap.map((piece, index)=> {
                ...
                } else if(thisPieceBottom > this.state.gameBoardWidth - 1 + this.state.gameBoardTop&& this.state.canMoveDown === true){
                    pieceMap[ev.target.id].y = this.state.gameBoardBottom - pieceHeight;
                    this.setState({
                        pieceMap: [...pieceMap],
                    })
            ...
            this.state.pieceMap.map((piece, index)=> {
                ...
                } else if(thisPieceTop < 1 + this.state.gameBoardTop && this.state.canMoveUp === true){
                    pieceMap[ev.target.id].y = this.state.gameBoardTop;
                    this.setState({
                        pieceMap: [...pieceMap],
                    })
